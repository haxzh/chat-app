...
function UsersList({ searchKey }) {
  ...
  function formatName(user) {
    let fname = user.firstname ? user.firstname.at(0).toUpperCase() + user.firstname.slice(1).toLowerCase() : "";
    let lname = user.lastname ? user.lastname.at(0).toUpperCase() + user.lastname.slice(1).toLowerCase() : "";
    return fname + ' ' + lname;
  }

  function getData() {
    if (searchKey === "") {
      // Show only users who are in a chat with current user (exclude self)
      return allChats
        .map(chat => Array.isArray(chat.members) ? chat.members.find(mem => mem._id !== currentUser._id) : null)
        .filter((user, idx, arr) => user && arr.findIndex(u => u._id === user._id) === idx);
    } else {
      // Show search result from all users
      return allUsers.filter(user => {
        return (
